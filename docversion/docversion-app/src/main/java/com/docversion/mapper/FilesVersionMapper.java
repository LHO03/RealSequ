...
public interface FilesVersionMapper {
    ...
    /**
     * 고착된 추출 상태를 다시 시도 가능하게 되돌린다. (RD-SRS-5.1 재검사 지원)
     *
     * <p>{@code VersionTextService}는 상태가 UNSUPPORTED 또는 FAILED이면 재추출을 시도하지
     * 않고 즉시 null을 돌려준다. 같은 문서를 반복해 파싱하지 않으려는 의도지만, 그 결과
     * <b>한 번 실패한 버전은 영원히 판정 불가로 남는다</b>. 추출기를 고치거나 라이브러리를
     * 추가해도 기존 버전은 자동으로 다시 잡히지 않는다.
     *
     * <p>재검사 요청 시 이 메서드로 상태를 되돌리면 다음 검사에서 추출을 다시 시도한다.
     * 이미 EXTRACTED인 버전은 건드리지 않는다 — 캐시된 텍스트가 여전히 유효하므로
     * 다시 파싱할 이유가 없다.
     *
     * @return 되돌린 행 수(0이면 고착 상태가 아니었다는 뜻)
     */
    int resetTextStatusIfUnresolved(@Param("versionId") String versionId);
