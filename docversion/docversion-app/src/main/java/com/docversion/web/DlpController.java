...
public class DlpController {
    ...
    public Map<String, Object> rescan(Principal principal,
        ...
        // 요청 경로의 문서에 속한 버전인지 대조한다. (F6)
        //
        // 위 인가는 fileId에 대해서만 이루어진다. versionId는 그대로 통과하므로,
        // 자기 문서의 fileId에 남의 버전 식별자를 붙이면 그 버전의 검사가 되돌려진다.
        // 조회 API(getScan)는 이미 같은 대조를 하고 있었고 여기만 빠져 있었다.
        Map<String, Object> version = versions.findVersionForContent(versionId);
        if (version == null || !fileId.equals(String.valueOf(version.get("fileId")))) {
            throw new ResourceNotFoundException(
                    "해당 문서의 버전이 아닙니다: file=" + fileId + " version=" + versionId);
        }

        long now = Instant.now().getEpochSecond();

        // 고착된 추출 상태를 함께 되돌린다. (FULL 검사만 해당)
        //
        // VersionTextService는 text_status가 UNSUPPORTED/FAILED면 재추출 없이 null을
        // 돌려주므로, 한 번 추출에 실패한 버전은 재검사를 아무리 요청해도 계속
        // 판정 불가로 남았다. 추출기를 고치거나 라이브러리를 추가해도 마찬가지였다.
        //
        // 변경분 검사(DELTA)는 추출 텍스트가 아니라 비교 결과를 입력으로 쓰므로 해당 없다.
        boolean textReset = false;
        if (ScanScope.FULL.name().equals(scope)) {
            textReset = versions.resetTextStatusIfUnresolved(versionId) > 0;
        }
        ...
        return Map.of("ok", true, "status", "PENDING", "textReset", textReset);
    }
