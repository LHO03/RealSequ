...
public class RetentionPolicyService {
    ...
    public int applyToFile(String fileId, int minDays, int maxDays, int maxVersions, String actingUser) {
        ...
        // 1) DB 삭제(트랜잭션) — diff 캐시 + DLP 검사 이력 + 버전 행 + 감사 로그
        ...
        for (String key : toDeleteKeys) {
            deleteQuietly(key);
            // 추출 텍스트(9.4 비교와 5.x 검사가 공유하는 캐시)도 같은 자리에 있다.
            // 원본만 지우면 문서 본문이 평문으로 담긴 .txt가 그대로 남는다.
            // 보존 정책으로 버전을 지우면서 본문 평문을 남겨두면 정리의 의미가 없다.
            deleteQuietly(VersionTextService.textKeyOf(key));
        }
        return toDeleteIds.size();
    }

    /** 없으면 조용히 넘어가고, 실패해도 정리 전체를 멈추지 않는다(롤백 불가 자원). */
    private void deleteQuietly(String storageKey) {
        try {
            storage.deleteFile(storageKey);
        } catch (RuntimeException e) {
            log.warn("[보존] 스토리지 삭제 실패(고아 파일 잔존): {} — {}", storageKey, e.getMessage());
        }
    }
    ...
    private List<String> resolveScopeFiles(String scopeType, String scopeId) {
        switch (scopeType) {
            ...
            case "FOLDER": return mapper.filesByFolderPrefix(escapeLikePrefix(scopeId));
    ...
    /**
     * LIKE 접두사에서 와일드카드를 무력화한다. (F16)
     *
     * <p>보존 정책의 FOLDER 범위는 경로 접두사로 대상 문서를 고른다. 이스케이프하지 않으면
     * 사용자가 넣은 {@code %}와 {@code _}가 와일드카드로 해석되어 의도보다 넓게 잡힌다.
     * 대상 범위가 {@code %} 하나면 <b>전체 문서</b>가 정리 대상이 된다.
     * 보존 정책은 버전을 삭제하는 기능이므로 이 오해석은 곧 의도하지 않은 삭제다.
     *
     * <p>역슬래시를 먼저 바꾸는 순서가 중요하다. 나중에 바꾸면 앞서 넣은 이스케이프 문자를
     * 다시 이스케이프해 원래 의미가 깨진다.
     *
     * <p>매퍼의 {@code ESCAPE '\'} 절과 짝이다. 한쪽만 바꾸면 동작하지 않는다.
     */
