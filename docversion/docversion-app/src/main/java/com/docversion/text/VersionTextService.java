...
public class VersionTextService {
    ...
    private String extractAndStore(String versionId, FileContent content) {
        ...
        // 선언된 MIME이 아니라 내용까지 보고 판단한다. 업로드 클라이언트가 붙이는
        // Content-Type은 믿을 수 없다 — curl은 확장자 표에 없는 .docx에
        // application/octet-stream을 붙이며, 그 값을 그대로 믿으면 Word 문서가
        // 통째로 판정 불가가 된다(탐지율 측정에서 docx 15건 전부 그렇게 나왔다).
